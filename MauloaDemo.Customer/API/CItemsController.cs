...
namespace MauloaDemo.Customer.Api {
    ...
    public class CItemsController : BaseApiController {

        private CItemRepository db = new CItemRepository();
        ...
        //[AllowAnonymous]
        //[HttpGet]
        //[Route("api/Citems/detail")]
        //[ResponseType(typeof(CItem))]
        ...
        [AllowAnonymous]
        [HttpGet]
        [Route("api/Citems/ChurchAvail")]
        [ResponseType(typeof(IEnumerable<ChurchAvail>))]
        public IHttpActionResult ChurchAvail(string id,
                                    DateTime wed_date)
        {
            try
            {
                ...
                {
                    var list = db.GetChurchAvail(id, wed_date).ToList();
                    foreach (var avail in list)
                    {
                        //購入不可の時間帯は画面表示しない
                        if (avail.status == "×")
                        {
        ...
        //        else
        //        {
        //            item = await db.FindItem(id, lang, wed_date);
        //            if (item == null) return NotFound();
        //        }
        //    }
        //    catch (Exception ex)
