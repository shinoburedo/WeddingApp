...
{
    public class ScheduleNotesController : BaseApiController {

        private ScheduleNoteRepository db = new ScheduleNoteRepository();

        [HttpGet]
        [Route("api/ScheduleNotes/Search")]
        [ResponseType(typeof(IEnumerable<ScheduleNote>))]
        ...
        [HttpGet]
        [Route("api/ScheduleNotes/GetNotesFromPatternId")]
        [ResponseType(typeof(IEnumerable<ScheduleNote>))]
        public async Task<IHttpActionResult> GetNotesFromPatternId(string c_num, int sch_pattern_id) {
            var list = await Task.Run(() => db.GetNotesFromPatternId(c_num, sch_pattern_id, _loginUser));
            return Ok(list);
        }

        [HttpGet]
        [Route("api/ScheduleNotes/{id?}")]
        [ResponseType(typeof(ScheduleNote))]
        public async Task<IHttpActionResult> Get(int? id) {
            ScheduleNote phrase = null;
            try {
                if (id == null || id == 0) {
                    phrase = new ScheduleNote();
                } else {
        ...
        [HttpPost]
        [Route("api/ScheduleNotes/Save")]
        [ResponseType(typeof(string))]
        public async Task<IHttpActionResult> Save(ScheduleNote phrase) {
            ...
            try {
                ...
                this.Validate<ScheduleNote>(phrase);
        ...
        [HttpPost]
        [Route("api/ScheduleNotes/SaveList")]
        [ResponseType(typeof(string))]
        public async Task<IHttpActionResult> SaveList(IEnumerable<ScheduleNote> list) {
            var status = string.Empty;
            ...
            foreach (var model in list) {
                ...
                //ここで再度検証を実行。(エラーがあればエラーメッセージを返す。)
                this.Validate<ScheduleNote>(model);
                if (!ModelState.IsValid) {
                    return Ok(buildModelStateErrorMsg());
                }
            }
        ...
        [HttpPost]
        [Route("api/ScheduleNotes/Delete/{id}")]
        [ResponseType(typeof(string))]
